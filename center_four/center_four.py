...
def render_and_save_image(viewport, output_path, image_width, image_height):
    try:
        success = viewport.saveAsImageFile(output_path, image_width, image_height)
        if not success:
...
def run(context):
    try:
        ...
        desired_horizontal_fov = 150  # degrees
        desired_vertical_fov = desired_horizontal_fov  # Make vertical FOV equal to horizontal FOV

        # Verify that Fusion 360 supports the desired vertical FOV
        max_vertical_fov = 160  # Maximum vertical FOV supported by Fusion 360
        if desired_vertical_fov > max_vertical_fov:
            ui.messageBox(f"Desired vertical FOV of {desired_vertical_fov} degrees exceeds maximum supported FOV of {max_vertical_fov} degrees.")
            return

        # Image dimensions (square images)
        image_size = 800  # Adjust as needed
        ...
        for position in positions:
            for heading in headings:
                ...
                # Left eye view
                left_heading_offset = EYE_OFFSET_ANGLE  # +50 degrees
                left_target = calculate_view_target(position, heading, PITCH_ANGLE, left_heading_offset)
                set_camera_for_eye(viewport, left_eye, left_target, desired_vertical_fov)
                time.sleep(0.5)
                output_path = os.path.join(
                    OUTPUT_DIR, 
                    f'left_x{position[0]/2.54:.2f}_y{position[1]/2.54:.2f}_h{heading}.png'
                )
                render_and_save_image(viewport, output_path, image_width, image_height)
    
                # Right eye view
                right_heading_offset = -EYE_OFFSET_ANGLE  # -50 degrees
                right_target = calculate_view_target(position, heading, PITCH_ANGLE, right_heading_offset)
                set_camera_for_eye(viewport, right_eye, right_target, desired_vertical_fov)
                time.sleep(0.5)
                output_path = os.path.join(
                    OUTPUT_DIR, 
